# VitaLink TeleHealth Solutions - Healthcare Alert Monitoring Dashboard

## 📊 Project Overview

This project presents an interactive **Healthcare Alert Monitoring Dashboard** developed in Tableau to analyse patient monitoring alerts, clinician response performance, device related factors, and operational trends within a telehealth environment.

The dashboard transforms healthcare monitoring data into actionable insights to support timely clinical intervention, improve alert management, and enhance operational decision making.

## 🖥️ Dashboard Preview

### Dashboard 1 – Clinical Response Performance Dashboard

<img width="810" height="400" alt="Vitalink Telehealth Solutions Clinical Response Performance Dashboard" src="https://github.com/user-attachments/assets/01505fb8-34cc-4f0d-b685-d400b3a0beb5" />

### Dashboard 2 – Patient Safety Device Monitoring Dashboard

<img width="815" height="402" alt="Vitalink Telehealth Solutions Patient Safety Device Monitoring Dashboard" src="https://github.com/user-attachments/assets/6751fccb-e887-4e2f-b39f-ce3ec24b8bec" />

## 🎯 Project Objectives

The analysis was designed to:

- Analyse the distribution of critical, warning and normal alerts.
- Identify the vital sign types generating the highest volume of system alerts.
- Examine how critical alerts are distributed across different patient age groups and primary health conditions.
- Evaluate clinician workload and compliance with the required 2-hour clinical response time.
- Identify alerts that breached the mandatory response time threshold.
- Compare alert patterns across different geographical locations.
- Investigate whether device battery levels and connectivity types are associated with warning alerts.

## 🛠️ Tools & Technologies

- **Tableau:** Data analysis, calculated fields, KPI development, interactive visualisation, and dashboard creation.
- **Data Visualisation:** Communicating healthcare performance and operational trends through charts and KPIs.
- **Healthcare Data Analytics:** Analysing clinical alerts, response performance, patient characteristics, and device related factors.

## 📈 Key Performance Indicators

The dashboard monitors several important performance indicators, including:

- Total Alerts Generated
- Critical Alerts
- Warning Alerts
- Average Clinician Response Time
- Alerts Breaching the 2-Hour Response Threshold
- Clinical Response Compliance Rate
- Alert Distribution by Vital Sign Type
- Alert Distribution by Patient Age Group
- Clinician Alert Workload

## 🔍 Key Areas of Analysis

### Alert Severity

Analysis of Critical, Warning, and Normal alerts to understand the overall distribution of healthcare monitoring events and identify areas requiring greater clinical attention.

### Vital Sign Monitoring

Evaluation of alert volumes generated by monitored vital signs, including Blood Pressure, Glucose, Pulse, and Oxygen levels.

### Patient Risk Analysis

Assessment of critical alerts across patient age groups and primary health conditions, including Diabetes and Hypertension, to identify higher-risk patient segments.

### Clinician Performance

Analysis of individual clinician alert workload, average response times, and compliance with the 2-hour clinical response requirement.

### Device & Connectivity Analysis

Investigation of warning alerts in relation to device battery percentages and connectivity methods, including Cellular and Bluetooth, to identify potential technology related patterns.

### Regional Analysis

Comparison of critical alert volumes across patient locations to identify geographical areas contributing disproportionately to critical alerts.

## 💡 Recommendations

- Prioritise critical alerts and high risk patient groups to support timely clinical intervention.
- Closely monitor compliance with the 2-hour response requirement and investigate recurring response delays.
- Review clinician workloads to identify potential workload imbalances that may affect response performance.
- Monitor device battery levels and connectivity issues to reduce avoidable technology related alerts.
- Use regional alert patterns to support resource allocation and targeted operational interventions.
- Continue monitoring key performance indicators through interactive dashboards to support data driven healthcare decision making.

## 📁 Project File

The complete Tableau Packaged Workbook (`.twbx`) is included in this repository:

**VitaLink Healthcare Alert Monitoring Dashboard.twbx**

## 🚀 Skills Demonstrated

- **Healthcare Data Analysis:** Analysed patient monitoring and clinical alert data to identify patterns in alert severity, patient risk, clinician performance, and operational response.

- **Tableau Dashboard Development:** Designed an interactive healthcare monitoring dashboard to communicate key clinical and operational performance indicators clearly.

- **KPI Development:** Developed and analysed KPIs including total alerts, critical and warning alerts, average response time, response time breaches, and clinical compliance rate.

- **Calculated Fields & Performance Metrics:** Created Tableau calculated fields to measure clinician response performance, alert volumes, and compliance with the required 2-hour clinical response threshold.

- **Data Visualisation & Storytelling:** Selected appropriate visualisations to communicate trends across patient demographics, vital signs, geographical locations, clinicians, and device related factors.

- **Clinical & Operational Insight:** Translated analytical findings into practical recommendations focused on alert prioritisation, clinician workload, response time compliance, patient risk, and operational efficiency.

- **Dashboard Design:** Applied clear visual hierarchy, KPI presentation, filtering, and interactive analysis to make healthcare performance information accessible to decision makers.

- **Data Driven Decision Making:** Converted complex healthcare monitoring data into actionable insights and recommendations to support informed clinical and operational decisions.

## 📌 Project Context

This project simulates a real world healthcare analytics engagement for VitaLink TeleHealth Solutions, a fictional telehealth organisation. The analysis applies healthcare data analytics and Tableau to evaluate patient monitoring alerts, clinician response performance, patient risk, device-related factors, and operational efficiency.

Working with a synthetic healthcare monitoring dataset designed to reflect realistic telehealth scenarios, I developed KPIs, calculated performance metrics, investigated clinical and operational trends, and designed interactive dashboards to translate complex data into actionable insights.

The project demonstrates my ability to approach healthcare data from an analyst’s perspective by identifying performance gaps, evaluating response-time compliance, prioritising patient safety risks, and communicating data driven recommendations to support clinical and operational decision making.

Data Note: The dataset used in this project is a fictional/synthetic healthcare dataset created for analytical and portfolio purposes. It does not contain real patient information.
